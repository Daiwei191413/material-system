-- V3.0.0 数据库 Schema（v3-legacy 改造版：双库分离）
-- 技象科技 BOM 整理神器 - 团队版
...
-- 唯一约束：同一库内 lcsc_code 唯一；两个库可以有同 lcsc_code 的不同记录
-- ============================================================
CREATE TABLE IF NOT EXISTS material_library (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    lib_type TEXT NOT NULL CHECK (lib_type IN ('lcsc', 'standard')),

    -- 立创编号 / 唯一定位字段（V2 中文字段：立创编号）
    ...
    -- 同一库内 lcsc_code 唯一；两个库分开
    UNIQUE (lib_type, lcsc_code)
);
...
CREATE TABLE IF NOT EXISTS audit_log (
    ...
    target TEXT,           -- 资源标识（lcsc_code 或 batch-时间戳 等）
